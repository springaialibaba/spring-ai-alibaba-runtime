...
import runtime.engine.memory.model.MessageContent;
import io.a2a.A2A;
...
public class GraphAgentExecutor implements AgentExecutor {
    ...
    public void execute(RequestContext context, EventQueue eventQueue) throws JSONRPCError {
        try {
            ...
            // 记录用户输入到会话与长期记忆
            runtime.engine.memory.model.Message userMsg = buildTextMessage(getTextFromMessageParts(message));
            ...
            // 检索相关记忆并拼接到输入前缀
            java.util.List<runtime.engine.memory.model.Message> retrieved = memoryService.searchMemory(
                    userId,
            ...
            if (!accumulatedOutput.isEmpty()) {
                runtime.engine.memory.model.Message assistantMsg = buildTextMessage(accumulatedOutput.toString());
    ...
    private static runtime.engine.memory.model.Message buildTextMessage(String text) {
        MessageContent content = new MessageContent("text", text);
        return new runtime.engine.memory.model.Message(MessageType.MESSAGE, java.util.List.of(content));
    }

    private static String formatRetrievedText(java.util.List<runtime.engine.memory.model.Message> messages) {
        ...
        int idx = 1;
        for (runtime.engine.memory.model.Message m : messages) {
    ...
    private static String extractTextFromMessage(runtime.engine.memory.model.Message message) {
